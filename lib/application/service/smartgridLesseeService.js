...
class Service {
    ...
    getLessees(lesseeID, traceContext, callback) {
        this._httpSmartgridLesseeServiceGateway.getLessees(lesseeID, traceContext, (err, lesseesJSON) => {
                if (err) {
                    callback(err, false);
                    return;
                }
    ...
    getStations(stationID, traceContext, callback) {
        this._httpSmartgridLesseeServiceGateway.getStations(stationID, traceContext, (err, stationsJSON) => {
                if (err) {
                    callback(err, false);
                    return;
                }
    ...
    getDataSources(dataSourceID, traceContext, callback) {
        this._httpSmartgridLesseeServiceGateway.getDataSources(dataSourceID, traceContext, (err, dataSourcesJSON) => {
                if (err) {
                    callback(err, false);
                    return;
                }
    ...
    getPermission(permissionID, traceContext, callback) {
        this._httpSmartgridLesseeServiceGateway.getPermission(permissionID, traceContext, (err, permissionsJSON) => {
                if (err) {
                    callback(err, false);
                    return;
                }
