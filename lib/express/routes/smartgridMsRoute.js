...
router.post('/roles', (req, res) => {
    ...
    let smartgridLesseeService = req.app.get('smartgridLesseeService');
    let traceContext = traceContextFeach(req);
    smartgridLesseeService.addRole(roleData, traceContext, (err, isSuccess) => {logger.error(roleData, traceContext);
        if (err) {
            logger.error(err.message, traceContext);
            return;
        }
...
router.get('/roles/:roleID', (req, res) => {
    ...
    let smartgridLesseeService = req.app.get('smartgridLesseeService');
    let traceContext = traceContextFeach(req);
    smartgridLesseeService.getRole(roleID, traceContext, (err, role) => {
        if (err) {
            logger.error(err.message, traceContext);
            return;
        }
...
router.get('/roles', (req, res) => {
    ...
    let smartgridLesseeService = req.app.get('smartgridLesseeService');
    let traceContext = traceContextFeach(req);
    smartgridLesseeService.getRoles(traceContext, (err, roles) => {
        if (err) {
            logger.error(err.message, traceContext);
            return;
        }
...
router.delete('/roles/:roleID', (req, res) => {
    ...
    let smartgridLesseeService = req.app.get('smartgridLesseeService');
    let traceContext = traceContextFeach(req);
    smartgridLesseeService.delRole(roleID, traceContext, (err, isSuccess) => {
        if (err) {
            logger.error(err.message, traceContext);
            return;
        }
...
router.post('/roles/:roleID/permissions', (req, res) => {
    ...
    let smartgridLesseeService = req.app.get('smartgridLesseeService');
    let traceContext = traceContextFeach(req);
    smartgridLesseeService.assignPermissionToRole(permissionID, roleID, traceContext, (err, isSuccess) => {
        if (err) {
...
router.delete('/roles/:roleID/permissions', (req, res) => {
    ...
    let smartgridLesseeService = req.app.get('smartgridLesseeService');
    let traceContext = traceContextFeach(req);
    smartgridLesseeService.canclePermissionofRole(permissionID, roleID, traceContext, (err, isSuccess) => {
        if (err) {
            resultJSON.errcode = errCodeTable.ERR.errCode;
            resultJSON.errmsg = errCodeTable.ERR.errMsg;
            resultJSON.isSuccess = false;
