...
router.get('/stations', (req, res)=> {
    ...
    let smartgridLesseeService = req.app.get('smartgridLesseeService');
    let traceContext = traceContextFeach(req);
    smartgridLesseeService.getStations(stationID, traceContext, (err, stations)=> {
        if (err) {
            logger.error(err.message, traceContext);
            return;
        }
...
router.get('/permissions/:permissionID', (req, res)=> {
    ...
    let permissionService = req.app.get('smartgridLesseeService');
    let traceContext = traceContextFeach(req);
    permissionService.getPermission(permissionID, traceContext, (err, permission)=> {
        if (err) {
            logger.error(err.message, traceContext);
            return;
        }
...
router.get('/permissions', (req, res)=> {
    ...
    let permissionService = req.app.get('smartgridLesseeService');
    let traceContext = traceContextFeach(req);
    permissionService.getPermissions(traceContext, (err, permissions)=> {
        if (err) {
            logger.error(err.message, traceContext);
            return;
        }
